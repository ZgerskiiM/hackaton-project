  ...
  <v-card flat class="todo-list">
    ...
    <v-card-text>
      <div class="d-flex align-center">
        <v-date-picker v-model="selectedDate" class="mr-4 mt-0" hide-header max-height="500"
          bg-color="transparent"></v-date-picker>
        <v-list class="flex-grow-1">
          <v-list-item v-for="(task, index) in tasksForSelectedDate" :key="index">
            <v-list-item-content>
              <v-list-item-title class="d-flex align-center">
                <input type="text" v-model="task.text" class="task-input" @blur="updateTask(index, task)"
                  @keypress.enter="updateTask(index, task)" />
                <v-checkbox v-model="task.completed" class="custom-checkbox"></v-checkbox>
              </v-list-item-title>
            </v-list-item-content>
          </v-list-item>
          <input type="text" v-model="newTaskText" placeholder="Новая задача" class="task-input ml-2" />
